...
int main(void)
{
	...
	*/
	...
	// Ввод времени
	cout << "Введите пожалуйста время после которого вы хотите увидеть маршируты: ";
	cin >> tmpDepartureTime;
	...
	for (int i = 0, count = 0; i <= 8; i++)
	{
		...
		if ((i == 8) && count == 0)
			cout << "Поездов после " << tmpDepartureTime << " больше не ходят" << endl << endl;
		...
		else if (train[i] > tmpDepartureTime) {
